...
namespace CreditBase
{
	class SqlLoader75 : SqlLoader
	{
		public SqlLoader75()
		{
			...
			srv = "R75-DBS-04.main.russianpost.ru";
			dbname = "r75-asku-work";
			usr = "temp";
			pswd = "temp";
		}
		...
		{
			NomLoader NL = SqlLoader75.NomLoader.Create(srv, dbname, usr, pswd, reg);
			NL.UpdateLocalNom();
		}
		...
		public override string WhatAPost(string nameFile)
		{
			...
			if (nameFile.Contains("report_ОСП Агинский почтамт")) pref = "65";
			if (nameFile.Contains("report_ОСП Борзинский почтамт")) pref = "66";
			if (nameFile.Contains("report_ОСП Краснокаменский почтамт")) pref = "67";
			if (nameFile.Contains("report_ОСП Могочинский почтамт")) pref = "68";
			if (nameFile.Contains("report_ОСП Нерчинский почтамт")) pref = "69";
			if (nameFile.Contains("report_ОСП Петровск-Забайкальский почтамт")) pref = "70";
			if (nameFile.Contains("report_ОСП Приаргунский почтамт")) pref = "71";
			if (nameFile.Contains("report_ОСП Улетовский почтамт")) pref = "72";
			if (nameFile.Contains("report_ОСП Читинский почтамт")) pref = "73";
			if (nameFile.Contains("report_ОСП Шилкинский почтамт")) pref = "74";
		...
		public override void LoadPricesDictionary()
		{
			...
			string defaultPriceKind = "B3DA2C59E545813F11E3E7B615F5CAAA";
